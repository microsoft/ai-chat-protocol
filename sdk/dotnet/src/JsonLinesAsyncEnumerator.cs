...
namespace Microsoft.AI.ChatProtocol
{
    ...
    {
        /// <summary>
        /// Enumerates a stream of JSON lines from the specified stream, deserializing each line into an instance of <see cref="ChatCompletionDelta"/>.
        /// </summary>
        /// <param name="stream">The stream to read from.</param>
        /// <param name="elementDeserializer">The function to use to deserialize each JSON line into an instance of <see cref="ChatCompletionDelta"/>.</param>
        ...
        internal static async IAsyncEnumerable<ChatCompletionDelta> EnumerateFromStream(
            Stream? stream,
            Func<JsonElement, ChatCompletionDelta> elementDeserializer,
            ILogger? logger,
            ...
            try
            {
                ...
                while (!cancellationToken.IsCancellationRequested && !done)
                {
                    ...
                    if (!string.IsNullOrEmpty(jsonLine))
                    {
                        ...
                        ChatCompletionDelta update = elementDeserializer.Invoke(sseMessageJson.RootElement);
