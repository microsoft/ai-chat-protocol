...
export const chatDefaultOptions: ChatComponentOptions = {
  ...
  promptSuggestions: [
    ...
    "What ingredients do I need for a banana cake?",
...
export class ChatComponent extends LitElement {
  ...
  onCitationClicked(citation: string) {
    if (this.options.enableContentLinks) {
      this.options.onCitationClicked?.(citation);
    }
  }
        ...
        <div class="suggestions">
          ${map(
            ...
            (suggestion) => html`
              <button
                class="suggestion"
                @click=${() => this.onSuggestionClicked(suggestion)}
              >
                ${suggestion}
              </button>
            `,
  ...
  protected renderError = () => html`
    <div class="message assistant error">
      <div class="message-body">
        ...
        <button @click=${() => this.onSendClicked(true)}>
          ${this.options.strings.retryButton}
        </button>
      </div>
    </div>
  `;
  ...
  protected renderCitationReference = (_citation: string, index: number) =>
    html`<sup>[${index}]</sup>`;
    ...
    return questions.length > 0
      ? html`
          <div class="questions">
            <span
              class="question-icon"
              title=${this.options.strings.followUpQuestionsTitle}
            >
            ...
            >${map(
              ...
              (question) => html`
                <button
                  ...
                  @click=${() => this.onSuggestionClicked(question)}
                >
                  ${question}
                </button>
              `,
      ...
      <div class="chat-input">
        ...
        <form class="input-form">
          <textarea
            ...
            .value=${this.question}
            autocomplete="off"
            @input=${(event: Event) => (this.question = (event.target as HTMLTextAreaElement).value)}
  ...
  protected override render() {
    const parsedMessages = this.messages.map((message) =>
      parseMessageIntoHtml(message, this.renderCitationReference),
    );
    return html`
      <section class="chat-container">
        ...
        this.messages.length === 0
          ...
          : nothing}
        <div class="messages">
          ${repeat(parsedMessages, (_, index) => index, this.renderMessage)}
          ${this.renderLoader()} ${this.hasError ? this.renderError() : nothing}
          ${this.renderFollowupQuestions(
            parsedMessages.at(-1)?.followupQuestions ?? [],
          )}
        </div>
  ...
  static override styles = css`
    :host {
      /* Base properties */
      --primary: var(--mai-primary, #07f);
      --error: var(--mai-error, #e30);
      --text-color: var(--mai-text-color, #000);
      --text-invert-color: var(--mai--text-invert-color, #fff);
      --disabled-color: var(--mai-disabled-color, #ccc);
      --bg: var(--mai-bg, #eee);
      --card-bg: var(--mai-card-bg, #fff);
      --card-shadow: var(
        --mai-card-shadow,
        0 0.3px 0.9px rgba(0 0 0 / 12%),
        0 1.6px 3.6px rgba(0 0 0 / 16%)
      );
      --space-md: var(--mai-space-md, 12px);
      --space-xl: var(--mai-space-xl, calc(var(--space-md) * 2));
      --space-xs: var(--mai-space-xs, calc(var(--space-md) / 2));
      --space-xxs: var(--mai-space-xs, calc(var(--space-md) / 4));
      --border-radius: var(--mai-border-radius, 16px);
      --focus-outline: var(--mai-focus-outline, 2px solid);
      --overlay-color: var(--mai-overlay-color, rgba(0 0 0 / 40%));

      /* Component-specific properties */
      --error-color: var(--mai-error-color, var(--error));
      --error-border: var(--mai-error-border, none);
      --error-bg: var(--mai-error-bg, var(--card-bg));
      --retry-button-color: var(--mai-retry-button-color, var(--text-color));
      --retry-button-bg: var(--mai-retry-button-bg, #f0f0f0);
      --retry-button-bg-hover: var(--mai-retry-button-bg, #e5e5e5);
      --retry-button-border: var(--mai-retry-button-border, none);
      --suggestion-color: var(--mai-suggestion-color, var(--text-color));
      --suggestion-border: var(--mai-suggestion-border, none);
      --suggestion-bg: var(--mai-suggestion-bg, var(--card-bg));
      --suggestion-shadow: var(
        --mai-suggestion-shadow,
        0 6px 16px -1.5px rgba(141 141 141 / 30%)
      );
      --user-message-color: var(
        --mai-user-message-color,
        var(--text-invert-color)
      );
      --user-message-border: var(--mai-user-message-border, none);
      --user-message-bg: var(--mai-user-message-bg, var(--primary));
      --bot-message-color: var(--mai-bot-message-color, var(--text-color));
      --bot-message-border: var(--mai-bot-message-border, none);
      --citation-color: var(--mai-citation-color, var(--text-invert-color));
      --bot-message-bg: var(--mai-bot-message-bg, var(--card-bg));
      --citation-bg: var(--mai-citation-bg, var(--primary));
      --citation-bg-hover: var(
        --mai-citation-bg,
        color-mix(in srgb, var(--primary), #000 10%)
      );
      --new-chat-button-color: var(
        --mai-button-color,
        var(--text-invert-color)
      );
      --new-chat-button-bg: var(--mai-new-chat-button-bg, var(--primary));
      --new-chat-button-bg-hover: var(
        --mai-new-chat-button-bg,
        color-mix(in srgb, var(--primary), #000 10%)
      );
      --chat-input-color: var(--mai-chat-input-color, var(--text-color));
      --chat-input-border: var(--mai-chat-input-border, none);
      --chat-input-bg: var(--mai-chat-input-bg, var(--card-bg));
      --submit-button-color: var(--mai-button-color, var(--primary));
      --submit-button-border: var(--mai-submit-button-border, none);
      --submit-button-bg: var(--mai-submit-button-bg, none);
      --submit-button-bg-hover: var(--mai-submit-button-color, #f0f0f0);
    }
    *:focus-visible {
    ...
    .chat-input {
      ...
      background: var(--bg);
      box-shadow: 0 calc(-1 * var(--half-space-xl)) var(--half-space-xl)
        var(--bg);
      display: flex;
      gap: var(--space-md);
    }
