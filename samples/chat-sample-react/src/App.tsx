...
import { Button, FluentProvider, Input, Text, ToggleButton, makeStyles, webLightTheme } from '@fluentui/react-components';
...
function Chat() {
  const client = new AIChatProtocolClient('http://localhost:8080/chat', {allowInsecureConnection: true});
  const configMessage: AIChatMessage = { role: 'system', content: 'You are an enthusiastic assistant that talks like a 5 year old.' };
  ...
  return (
    <div>
        ...
        <ToggleButton checked={streaming} onClick={(e) => setStreaming(!streaming)} >Streaming</ToggleButton>
        </div>
    </div>
  );
}
